...
Posledná rovnica je splnená práve vtedy, keď $z = -x$, vtedy však $y = -2x - 2z = 0$.Dostávame trojice $(x, y, z) = (x, 0, -x)$, ktoré sú riešeniami danej sústavy pre každé reálne $x$, ako overíme dosadením. (O takej skúške platí to isté čo v prípade \todo{a)}.)
\end{enumerate}
...
Platí teda $z = -x$ alebo $z = x$. Dosadením do rovnosti $x + z - 2y = 0$ v prvom prípade dostaneme $y = 0$, v druhom prípade $y = x$. Zodpovedajúce trojice $(x, 0, -x)$ a $(x, x, x)$ sú riešeniami pre každé reálne $x$ (prvé z nich sme však našli už v časti \todo{a)}).
\end{enumerate}
}

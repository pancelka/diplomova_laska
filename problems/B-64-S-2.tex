...
\rieh Označme $S_{XYZ}$ obsah trojuholníka $XYZ$ vyjadrený v cm$^2$ a ďalej označme $S = S_{ABP}$. Podľa zadania platí $S_{ADP} = 10$, $S + S_{BCP} = 8$, $S_{BCP} + S_{CDP} = 9$. Z druhej rovnosti vyplýva $S_{BCP} = 8 - S$, dosadením do tretej rovnosti potom vyjde $S_{CDP} = 1 + S$ \todo{fixnimi (obr. 1)}.
